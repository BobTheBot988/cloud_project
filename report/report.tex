...
cost. The service runs on two \code{t3.medium} workers
...
\ref{sec:campaignB} the two campaigns, and Sec.~\ref{sec:cost} the cost
evaluation.
...
\code{t3.small} inside the AWS perimeter.
...
Linux 2023, Kubernetes v1.36, Flannel, Metrics Server). Both variant campaigns
(exp4, exp6) ran on a single 6-worker cluster with the same control plane,
differing only in the HPA \code{maxReplicas}. The model is
...
\code{t3.small} in the same region. Every experiment ended with a full
...
the elasticity axis, though not the method. The lab's hard caps (8 instances /
31 vCPU) bound the variant campaign to $N{=}20$ and six workers, and its
temporary, region-scoped credentials meant runs span two regions
(us-east-1, us-west-2).
...
A methodological caveat applies to these runs and to the variant campaign:
there is no warm-up phase. Runs ramp users immediately (\code{-r 5}), so the
figures include cold starts and the HPA cold-starting pods mid-run after
scaling down between runs (level 30 of the variants: 6 of 20 runs started at
one pod). Delay and pod figures are therefore full autoscaling behaviour, not
clean steady state; Test~A, which has an explicit warm-up and idle drain, is
the clean case.
...
(exp4) and to 6 (exp6), re-running
the full intensity grid 10--50 users with $N{=}20$ per level. The cost of the
HPA reacting correctly is that it never had room to react; here it does. Both
variants used the same six-worker cluster, so the only difference between them
is the \code{maxReplicas} ceiling, and the same mechanism that produced the
clean $1{\to}2$ out-and-back cycle of Test~A now scales to the configured
maximum (4 and 6 pods, Fig.~\ref{fig:capacity_variant}) under sustained load.
...
absorbed by the service queue, sustained load is not. The three experiments
that use similar user counts are therefore not contradictory but occupy
different regimes. Test~B sustains a \emph{mixed} workload at 10--20 users on
two decode slots, so the queue grows until a fraction exceeds the 300\,s
timeout and errors reach 22--35\%. Test~D applies the same 12 users as a
\emph{burst} with a 120\,s lull, during which the queue drains, so no request
is lost. Test~C isolates a \emph{light} class (small, 32 tokens) at 20 users
and shows 1.2\% errors because each request is short. Sustained mixed load,
bursty load with idle recovery, and light isolated load are different points
on the same queueing curve, not conflicting measurements.
