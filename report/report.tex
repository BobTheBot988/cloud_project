...
users with a 2-minute steady phase, 10 runs per size; \code{exp-c.sh}.
...
most runs empty. We report it as a limitation (Sec.~\ref{sec:limits}) rather
than a result.
...
\emph{Test C large is underpowered.} Ten requests across ten runs, seven of
which were empty, because the run protocol (2-min steady, 20 users) is shorter
than the service time (~90--120\,s) at two slots. The large class is not
statistically comparable with the others; isolating it would need a 4-user
regime as the runbook recommended, which was not executed.
...
the queue. Size isolation showed the cost of a request scales with its length
and that a mixed queue penalises short requests ~3.5\,$\times$. The burst
