...
| `justfile` | recipes: test, test-prompt, up/down, launch/cluster-up/cluster-verify/cluster-down, kind-up/load/metrics/deploy/test/fast/down, case-0/1/2 + aliases, guard-default, case-all, exp-a/exp-b/collect/collect-stop/exp-smoke, exp4/exp6 (+ -up/launch4/launch6), plots |
...
- exp-a/exp-b remote branch: PATH-robust locust (`/tmp/exp/.venv/bin`), ssh hardened (ServerAlive + `</dev/null` + local `timeout` wrapper) after a 30-min ssh hang; `plots/analyze.py` built (variants); **exp-c built** (size-isolated Test C runner, `just exp-c`); exp-d/sanity/loadgen-up remain Person B-owned.
