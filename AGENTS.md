...
- exp-a/exp-b remote branch: PATH-robust locust (`/tmp/exp/.venv/bin`), ssh hardened (ServerAlive + `</dev/null` + local `timeout` wrapper) after a 30-min ssh hang; `plots/analyze.py` built (variants); exp-c/exp-d/sanity/loadgen-up are Person B-owned (not built yet).
