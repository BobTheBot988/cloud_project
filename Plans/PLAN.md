...
- **Test C** (size-isolated @20 users, 6 pod fissi): `data/raw/testC_{small,medium,large,mix}/` (10 run/scenario, drain 90s, restart pod a run 5) — **0 errori su 5.227 richieste**: small 1.56 req/s p50 7.4s, medium 0.51 req/s p50 30s, large 0.25 req/s p50 52s, mix 0.61 req/s. Collo di bottiglia = slot paralleli (12 = 6 pod × --parallel 2), non CPU; llama-server ha un memory leak (~2.9Gi/45min) che richiede restart periodico. Full details in `Plans/blocco_3_person_b.md`.
